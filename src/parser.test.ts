...
**First** and **second**`;
      ...
      expect(slides[0].elements[0]).toEqual({
        type: "textline",
        parts: [
          { type: "text", content: "First", bold: true },
          { type: "text", content: " and " },
          { type: "text", content: "second", bold: true },
        ],
      });
    });
  });
...
[First](url1) and [Second](url2)`;
      ...
      expect(slides[0].elements[0]).toEqual({
        type: "textline",
        parts: [
          { type: "link", text: "First", url: "url1" },
          { type: "text", content: " and " },
          { type: "link", text: "Second", url: "url2" },
        ],
      });
    });
  });
...
:::`;
  ...
  describe("code block parsing", () => {
    test("does not treat # as heading inside code blocks", () => {
      const markdown = `# Title
...
\`\`\``;
      ...
      expect(slides[0].elements[0]).toEqual({
        type: "code",
        language: "bash",
        content: "# Snapshot captures this\nalias rm='rm -i'  # Always prompts!",
      });
    });

    test("parses code blocks without language specifier", () => {
      const markdown = `# Title
...
\`\`\``;
    ...
    test("parses TypeScript code blocks", () => {
      const markdown = `# Title
...
![Image](file.txt)`;
      ...
      // Check we have heading, textline, bullets, and image
      const types = slides[0].elements.map((e) => e.type);
      expect(types).toContain("heading");
      expect(types).toContain("textline");
